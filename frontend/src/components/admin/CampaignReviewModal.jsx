    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
      <div className="relative w-full max-w-xl bg-white rounded-xl shadow-2xl border border-slate-200 overflow-hidden">
        ...
        <div className="p-6 space-y-5 text-slate-700 text-sm">
          ...
          <div className="space-y-2 pt-1">
            <div className="flex items-center justify-between">
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-700">
                Confirm recipient count (<span className="text-navy font-mono text-sm font-bold">{frozenCount}</span>):
              </label>
              <button
                type="button"
                onClick={() => setTypedCount(String(frozenCount))}
                className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold bg-amber-100 hover:bg-amber-200 text-amber-900 border border-amber-300 rounded transition-colors"
                title="Click to automatically fill the required count"
              >
                ⚡ Auto-fill "{frozenCount}"
              </button>
            </div>
            <div className="relative">
              <input
                ...
                placeholder={`Type ${frozenCount} or click Auto-fill above`}
              ...
              <div className="text-xs pt-0.5">
                {countMatches ? (
                  <span className="text-emerald-700 font-semibold flex items-center gap-1.5">
                    <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> Count verified: {frozenCount} recipients
                  </span>
                ) : (
                  <span className="text-amber-700 font-medium flex items-center gap-1.5">
                    <AlertTriangle className="w-3.5 h-3.5 text-amber-600" /> Count must match exactly: enter {frozenCount}
                  </span>
                )}
              </div>
            )}
          ...
          <label className="flex items-start gap-2.5 cursor-pointer text-xs text-slate-700 select-none bg-slate-50 p-3 rounded-lg border border-slate-200 hover:bg-slate-100 transition-colors">
            <input
              type="checkbox"
              checked={confirmedCheck}
              onChange={(e) => setConfirmedCheck(e.target.checked)}
              className="mt-0.5 rounded border-slate-300 text-navy focus:ring-navy cursor-pointer"
            />
            <span className="leading-relaxed">
              I confirm this email campaign is verified and ready to be delivered to all <strong>{frozenCount}</strong> recipients.
            </span>
          </label>
        </div>
        ...
        <div className="bg-slate-50 px-6 py-4 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            {!canDispatch && (
              <span className="text-[11px] text-amber-700 font-medium">
                {!countMatches
                  ? `Click "Auto-fill ${frozenCount}" above`
                  : !confirmedCheck
          ...
          <button
            ...
            className={`w-full sm:w-auto px-5 py-2 text-xs font-bold rounded-lg flex items-center justify-center gap-2 shadow-sm transition-all ${
              canDispatch
